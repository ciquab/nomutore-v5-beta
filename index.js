...
if (checkModal) {
    ...
    handleRepeat: async (log) => {
        try {
            if (log.type === 'beer') {
                // ビール保存イベントを発火
                // リスナー側で { data, existingId } を期待している構造に合わせる
                const event = new CustomEvent('save-beer', { 
                    detail: {
                        data: {
                            ...log,
                            timestamp: Date.now(),
                            date: dayjs().format('YYYY-MM-DD'), // 本日の日付
                            isCustom: log.isCustom || false,
                            useUntappd: false // リピート時はUntappdを開かない
                        },
                        existingId: null // 新規作成
                    } 
                });
                document.dispatchEvent(event);
            ...
            // UI更新などはイベントリスナー側で行われるため、ここでは何もしなくてOK
